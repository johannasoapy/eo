...
function custom_post_article() { 
	...
	register_post_type( 'article', /* (http://codex.wordpress.org/Function_Reference/register_post_type) */
		...
		array( 'labels' => array(
			...
			'rewrite'	=> array( 'slug' => 'article/%settlement-topic%', 'with_front' => false ), /* you can specify its url slug */
			'has_archive' => 'article', /* you can rename the slug here */
			'capability_type' => 'post',
              'taxonomies' => array( 'post_tag','settlement-topic' ),
			'hierarchical' => false,
...
// now let's add custom categories for Articles and Theme Pages (these act like categories)
	register_taxonomy( 'settlement-topic', 
		array('article','themepage','sfwd-courses','sfwd-lessons','forum','topic','sfwd-quiz','sfwd-topic'), /* if you change the name of register_post_type( 'article', then you have to change this */
	...
	*/	
	...
	register_taxonomy( 'language_level', 
		array('article','sfwd-courses', 'sfwd-lessons', 'sfwd-quiz', 'forum'), /* the name of the post type */
		array('hierarchical' => false,    /* if this is false, it acts like tags */
			'labels' => array(

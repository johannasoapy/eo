			...
			<div id="content">
				...
				<div id="inner-content" class="wrap cf">
					...
					<main id="main" class="m-all t-2of3 d-3of4 last-col cf" role="main">
						...
						<form class="refine-search" role="search" method="get" id="searchform" action="<?php //echo home_url( '/' ); ?>">
						<input type="text" name="s" id="s" <?php if(is_search()) { ?>value="<?php the_search_query(); ?>" <?php } else { ?>value="Enter keywords &hellip;" onfocus="if(this.value==this.defaultValue)this.value='';" onblur="if(this.value=='')this.value=this.defaultValue;"<?php } ?> />
							<div class="right checkboxes">
								...
								<fieldset style="display:none;"><label><input type="checkbox" name="post_type[]" value="themepage" <?php if (in_array('article', $query_types)) { echo 'checked'; } else { echo str_replace( 'checked','','checked'); } ?> />Themepage</label></fieldset>
						...
						<?php endwhile; ?>
							...
							<?php else : ?>
									...
									<article id="post-not-found" class="hentry cf">
										...
										<footer class="article-footer">
												<p><?php _e( 'We are in the process of creating a "Search Tips" post.', 'bonestheme' ); ?></p>
										</footer>
									</article>
